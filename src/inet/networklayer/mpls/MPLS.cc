...
bool MPLS::tryLabelAndForwardIPv4Datagram(IPv4Datagram *ipdatagram)
{
    ...
    if (!mplsPacket->hasLabel()) {
        ...
        delete mplsPacket;
        ipdatagram->ensureTag<ProtocolInd>()->setProtocol(&Protocol::ipv4);
...
void MPLS::processMPLSPacketFromL2(MPLSPacket *mplsPacket)
{
    ...
    if (mplsPacket->hasLabel()) {
        ...
        if (outgoingInterface) {
            ...
            nativeIP->ensureTag<ProtocolInd>()->setProtocol(&Protocol::ipv4);
            sendToL2(nativeIP);
        }
        else {
            sendToL3(nativeIP);
...
void MPLS::sendToL2(cMessage *msg)
{
    ...
    msg->getMandatoryTag<InterfaceReq>();
    msg->getMandatoryTag<ProtocolInd>();
    send(msg, "ifOut");
}
...
{
    ASSERT(msg->getControlInfo());
    msg->getMandatoryTag<InterfaceInd>();
    msg->getMandatoryTag<ProtocolReq>();
    send(msg, "netwOut");
}
