...
const ITransmissionBitModel* APSKEncoder::encode(const ITransmissionPacketModel* packetModel) const
{
    ...
    if (forwardErrorCorrection == nullptr)
        return new TransmissionBitModel(netHeaderBitLength, bitrate, serializedBits->getSize() - netHeaderBitLength, bitrate, encodedBits, forwardErrorCorrection, scrambling, interleaving);
    else {
